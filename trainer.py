...
class Trainer:
    def __init__(self, train_loader, dev_loader, test_loader, model: nn.Module, loss_fn, optimizer, scheduler, save_dir='.', display=100, eval=False, device='cuda', tensorboard=False, mode='both',type_=""):
    ...
    def train(self, max_iter):
        ...
        for idx_iter in range(max_iter):
            ...
            for data in tqdm(self.train_loader, total=len(self.train_loader)):
                ...
                if self.device != 'cpu':
                    with torch.cuda.amp.autocast():
                        logits = self.model(x)
                        logits = logits.float()  # 确保 logits 是浮点类型
                        y = y.long()  # 确保目标标签是长整型
                ...
                if batch % self.display == 0:
                    ...
                    display_total = 0
                    display_total_loss = 0
                print ("train:", y.cpu().numpy()[:100], indices.cpu().numpy()[:100])
            ...
            if dev_loss < best_dev_loss:
                self.model.module.save('best_'+self.type)
    ...
    def validate(self, idx_iter=0):
        ...
        for data in self.dev_loader:
            ...
            print ("validate: ", y.cpu().numpy()[:100], indices.cpu().numpy()[:100])
            all_predictions.extend(indices.cpu().numpy())
            all_labels.extend(y.cpu().numpy())
            """
    ...
    def predict(self):
        ...
        for data in self.test_loader:
            ...
            print (indices, y, sum(indices == y).item(), "res")
            correct += sum(indices == y).item()
        ...
        logging.info("classification_report: {}".format(report))
