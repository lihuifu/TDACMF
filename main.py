...
class CustomLoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        # 计算1-10数据的额外惩罚
        mask = (target >= 1) & (target <= 10)
        extra_loss = torch.where(mask, (pred - target) ** 2 * self.weight, 0.0)
        
        # 总损失
        total_loss = mse_loss + extra_loss.mean()
        return total_loss
...
if __name__ == '__main__':
    ...
    # set logger
    logging.basicConfig(filename=osp.join(save_dir, 'output_{}.log'.format(str(int(time.time())))) + "_"+type_, level=logging.INFO)
    ...
    #dev_set.initialize(opt, phase='train', cat='all',
                       task=TASK)
    ...
    #test_set.initialize(opt, phase='train', cat='all',
                        task=TASK)
    ...
    #class_weights = torch.tensor([0.1, 0.1, 0.2, 0.3, 0.1]).to(device)  # 根据类别分布调整
    #loss_fn = nn.CrossEntropyLoss(weight=class_weights)
    loss_fn = nn.CrossEntropyLoss()
    #loss_fn = CustomLoss(weight=5.0)
    if MODE == 'text_only':
    ...
    trainer = Trainer(train_load, dev_load, test_load,
                      model, loss_fn, optimizer, scheduler, eval=EVAL, device=device, tensorboard=USE_TENSORBOARD, mode=MODE, type_=type_)
