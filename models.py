...
from transformers import BertModel, BertConfig, AlbertModel, AlbertConfig, XLNetConfig, XLNetModel, ElectraConfig, ElectraModel, CLIPProcessor, CLIPModel, CLIPTextModel, CLIPTokenizer, AutoModel, AutoTokenizer, AutoConfig, AutoModel, AutoModelForCausalLM
import os
...
class TextOnlyModel(BaseModel):
    def __init__(self, save_dir, dim_text_repr=768, num_class=2):
        ...
        #self.textEncoder = AutoModel.from_pretrained('google/bigbird-roberta-base')
 
        # Longformer 模型导入, 效果最好 
        #config = AutoConfig.from_pretrained('allenai/longformer-base-4096')
        #self.textEncoder = AutoModel.from_pretrained('allenai/longformer-base-4096')
 
        # Reformer 模型导入
        #config = AutoConfig.from_pretrained('google/reformer-crime')
        #self.textEncoder = AutoModel.from_pretrained('google/reformer-crime')

        #config = AutoConfig.from_pretrained("Qwen/Qwen-7B")
        #self.textEncoder = AutoModel.from_pretrained("Qwen/Qwen-7B")

        config = AutoConfig.from_pretrained("microsoft/deberta-v3-base")
        self.textEncoder = AutoModel.from_pretrained("microsoft/deberta-v3-base")
...
class ImageOnlyModel(BaseModel):
    def __init__(self, save_dir, dim_visual_repr=512, num_class=2):
        super(ImageOnlyModel, self).__init__(save_dir=save_dir)

        #self.imageEncoder = torch.hub.load(
        #    'pytorch/vision:v0.8.0', 'resnet50', pretrained=True)
            #'pytorch/vision:v0.8.0', 'densenet201', pretrained=True)

        self.imageEncoder = CLIPModel.from_pretrained("openai/clip-vit-base-patch32")
    ...
    def forward(self, x):
        ...
        #f_i = self.flatten_vis(self.imageEncoder(image))
 
        f_i = self.dropout(self.flatten_vis(self.imageEncoder.get_image_features(pixel_values=image)))
...
class DenseNetBertMMModel(MMModel):
    ...
    def __init__(self, save_dir, dim_visual_repr=512, dim_text_repr=768, dim_proj=100, num_class=1):
        self.save_dir = save_dir
        ...
        imageEncoder = torch.hub.load(
            'pytorch/vision:v0.8.0', 'resnet101', pretrained=True)
            #'pytorch/vision:v0.8.0', 'densenet201', pretrained=True)
        """

        imageEncoder = CLIPModel.from_pretrained("openai/clip-vit-base-patch32")
        ...
        # 加载 Reformer 模型
        #model_name = "google/reformer-crime"
        #model = AutoModel.from_pretrained(model_name)
        #tokenizer = AutoTokenizer.from_pretrained(model_name)
        ...
        # BigBird 模型导入
        #config = AutoConfig.from_pretrained('google/bigbird-roberta-base')
        #textEncoder = AutoModel.from_pretrained('google/bigbird-roberta-base')

        # Longformer 模型导入, 效果最好
        config = AutoConfig.from_pretrained('allenai/longformer-base-4096')
        textEncoder = AutoModel.from_pretrained('allenai/longformer-base-4096')

        # Reformer 模型导入
        #config = AutoConfig.from_pretrained('google/reformer-crime')
        #textEncoder = AutoModel.from_pretrained('google/reformer-crime')
    ...
    def forward(self, x):
        ...
        # N, dim_visual_repr
        #image_features = self.imageEncoder.encode_image(image)
        #image_features = self.imageEncoder(image)
        #image_features = self.imageEncoder.encode_image(image)
        #f_i = self.dropout(self.flatten_vis(image_features))
